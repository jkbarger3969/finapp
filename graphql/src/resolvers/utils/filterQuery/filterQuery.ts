      ...
      case "or":
        if (value) {
          filterQuery.$or = await Promise.all(
            (value as unknown as NonNullable<LogicOperators<TWhere>["or"]>).map(
              (where) => filter(where, fieldAndConditionCreator)
            )
          );
        }
        break;
      case "and":
        if (value) {
          filterQuery.$and = await Promise.all(
            (
              value as unknown as NonNullable<LogicOperators<TWhere>["and"]>
      ...
      case "nor":
        if (value) {
          filterQuery.$nor = await Promise.all(
            (
              value as unknown as NonNullable<LogicOperators<TWhere>["nor"]>
            ).map((where) => filter(where, fieldAndConditionCreator))
          );
        }
        break;
