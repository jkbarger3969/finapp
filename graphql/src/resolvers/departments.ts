...
export const departments: QueryResolvers["departments"] = async (
  ...
  const searchByNameRegex =
    searchByName.length > 0 ? new RegExp(`(^|\\s)${searchByName}`, "i") : null;
  ...
  if (fromParent) {
    ...
    const parentPromise = db
      ...
      .aggregate([
        searchByNameRegex
          ? { $match: { ...fromMatch.$match, name: searchByNameRegex } }
          : fromMatch,
        addId
      ])
      .toArray();
    ...
    const nameOnlyResults = await db
      .collection("departments")
...
const ancestors: DepartmentResolvers["ancestors"] = async (
  parent,
  args,
  context,
  info
  ...
  if (parentNodeType.typename === "Department") {
    ...
    const businessParent = await nodeDocResolver(
      results[results.length - 1].parent,
      context
    );
  ...
  const businessParent = await nodeDocResolver({ node, id: docId }, context);
...
const budget: DepartmentResolvers["budget"] = async (
  parent,
  args,
  context,
  info
