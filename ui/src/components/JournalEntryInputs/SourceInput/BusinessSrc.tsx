...
import {
  BusinessSrcOptsInput_1Query as BusinessSrcOptsInputQuery,
...
const BUSINESS_SRC_OPTS_INPUT_QUERY = gql`
  query BusinessSrcOptsInput_1(
    $searchByName: String!
    $deptId: ID!
    $bizId: ID!
    $withBizOpt: Boolean!
    $withDeptAncestors: Boolean!
    $withBizOpts: Boolean!
  ) {
...
const BusinessSrc = function(props: BusinessSrcProps) {
  ...
  const [pullBizData, { loading: loadingNewBiz }] = useLazyQuery(
    INSURE_BIZ_DATA
  );
  ...
  const { loading, error, data } = useQuery<
    BusinessSrcOptsInputQuery,
    BusinessSrcOptsInputQueryVariables
  ...
  const srcVal = useMemo(() => {
    if (!src || !data) {
      ...
      try {
        return (
          client.readFragment<BusinessSrcDeptOptsFragment>(
            {
              id: `Business:${src.id}`,
  ...
  const valueChain = useMemo(() => {
    ...
    const value: (
      | BusinessSrcBizOptsFragment
      | BusinessSrcDeptOptsFragment
    )[] = [srcVal];

    const getParent = (
      srcVal: BusinessSrcBizOptsFragment | BusinessSrcDeptOptsFragment
    ): BusinessSrcBizOptsFragment | BusinessSrcDeptOptsFragment | null => {
  ...
  const options = useMemo(() => {
    ...
    return (bizVal?.deptOpts || []).filter(
      deptOpt => deptOpt.parent.id === srcVal.id
    );
  ...
  const filterOptions = useCallback(
    (opts: (BusinessSrcBizOptsFragment | BusinessSrcDeptOptsFragment)[]) => {
      const searchText = srcInput
        .trimStart()
        .split(" ")
        .join("\\s");
